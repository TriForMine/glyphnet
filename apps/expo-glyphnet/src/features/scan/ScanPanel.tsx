...
import { Modal, Platform, useColorScheme, Vibration } from "react-native";
...
export function ScanPanel() {
  ...
  const saveDebugBundle = async () => {
    ...
    try {
      ...
      const cropped = await ImageManipulator.manipulateAsync(lastCaptureUri, [{ crop }], {
        compress: 1,
    ...
    <View className="flex-1">
      {permission?.granted ? (
        <View className="relative h-full w-full">
          ...
          >
            <Text className="text-center text-lg font-semibold tracking-wide text-white">GlyphNet</Text>
          </View>
          <View
            pointerEvents="none"
            style={{
        ...
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-center text-sm text-slate-600 dark:text-neutral-300">{statusLabel}</Text>
          <Pressable
      ...
      >
        <View className="flex-1 items-center justify-center bg-black/65 p-5">
          <View className="w-full max-w-[560px] rounded-2xl bg-slate-900 p-4">
            <Text className="text-xs font-semibold uppercase tracking-wide text-slate-400">Scan Result</Text>
            {parsedResult ? (
              <View className="mt-2 gap-3">
                ...
                {parsedResult.ok ? (
                  <View className="rounded-xl bg-slate-800 p-3">
                    ...
                    <Text className="mt-1 text-xs text-slate-400">{parsedResult.payload_len ?? 0} bytes</Text>
                  </View>
                ) : (
                  <View className="rounded-xl bg-slate-800 p-3">
                ...
                {debugEnabled && lastCaptureDataUri ? (
                  <View className="rounded-xl bg-slate-800 p-3">
                    <Text className="text-xs uppercase tracking-wide text-slate-400">Debug Capture + ROI</Text>
